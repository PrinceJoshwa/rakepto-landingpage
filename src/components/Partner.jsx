...
const Partner = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // --- ADDED FOR FORMSPREE ---
    // This is a "fire-and-forget" submission.
    // It runs but doesn't block the Resend logic below.
    // The UI feedback (message, loading) remains tied to the Resend call.
    fetch("https://formspree.io/f/xkgpzveg", { // ⚠️ IMPORTANT: Replace with your Formspree ID
      method: "POST",
      body: JSON.stringify(form),
      headers: {
    ...
    // --- END FORMSPREE ADDITION ---

    // --- EXISTING RESEND LOGIC (Unchanged) ---
    try {
      const response = await fetch("/api/sendReferral", {
        method: "POST",
        headers: {
